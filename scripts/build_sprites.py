...
def load_manifest_icons(source: Path, manifest_path: Path) -> Dict[str, List[IconSpec]]:
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    groups: Dict[str, List[IconSpec]] = {}
    for item in payload.get("icons", []):
        group = str(item.get("group", "fallback"))
        name = str(item.get("name") or Path(str(item.get("file", "icon.png"))).stem)
        ...
        if not path.exists() or path.suffix.lower() != ".png":
            continue
        groups.setdefault(group, []).append(IconSpec(name=name, path=path))
    return groups
...
def build_sprite_group(group: str, icons: Sequence[IconSpec], out_dir: Path, scale: int) -> None:
    ...
    png_path = group_dir / f"sprite{suffix}.png"
    json_path = group_dir / f"sprite{suffix}.json"
...
def main() -> int:
    ...
    parser.add_argument("--manifest", default="", help="Optional icons.manifest.json generated by extract/convert steps.")
    ...
    groups = load_manifest_icons(source, manifest) if manifest and manifest.exists() else discover_groups(source)
    if not groups:
