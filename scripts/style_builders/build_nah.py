...
def add_nah_zone_layer(style, source_layer, source_id):
    base_id = f"{source_id}-zone-{source_layer}"
...
def add_nah_point_layer(style, source_layer, source_id):
    ...
    style["layers"].append({
        ...
        "layout": {
            ...
            "icon-size": ["interpolate", ["linear"], ["zoom"], 6, 0.45, 12, 0.75],
            "icon-anchor": "bottom",
...
def build_style(dataset_config, args):
    ...
    for g in sorted(geojsons):
        ...
        # Wir fügen IMMER beide Layer-Typen hinzu (Point und Polygon), 
        # MapLibre filtert diese anhand der Geometrie im Vektortile.
        # So stellen wir sicher, dass sowohl Pins als auch Zonen sichtbar sind.
        add_nah_zone_layer(style, source_layer, slug)
        add_nah_point_layer(style, source_layer, slug)
